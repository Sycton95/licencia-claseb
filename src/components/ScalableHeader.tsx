...
export function ScalableHeader({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className={`absolute inset-0 pointer-events-none ${isDarkMode ? 'opacity-20' : 'opacity-[0.05]'
          }`}
        style={{
      ...
      />
      <div
        className={`absolute inset-x-0 top-0 h-40 bg-gradient-to-b to-transparent pointer-events-none ${isDarkMode ? 'from-indigo-500/20' : 'from-indigo-500/10'
          }`}
      />
            ...
            <div className="flex flex-col items-center justify-between w-max max-w-full">
              ...
              <div className="flex flex-row items-center w-full" style={{ gap: '0.073em' }}>
                {[...Array(dashCount)].map((_, i) => (
                  <div
                    key={i}
                    className={`rounded-none transition-colors duration-300 ${isDarkMode ? 'bg-white' : 'bg-slate-900'
                      }`}
                    style={{
