...
export function createFactNormalizer({ unitNormalization }) {
  ...
  function extractEntities(text) {
    ...
    const results = [];
    const numberWithUnitPattern = /(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d+)?|\d+(?:[.,]\d+)?)(?:\s*[-Ã¢â‚¬â€œ]\s*(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d+)?|\d+(?:[.,]\d+)?))?\s*(km\/h|kmh|kms\/h|g\/l|gr\/l|utm|aÃƒÂ±os|anos|aÃƒÂ±o|ano|meses|mes|cm|cms|centimetro|centimetros|centÃƒÂ­metro|centÃƒÂ­metros|psi|p\.s\.i\.|gramos por mil|gramos por litro|gramos de alcohol por litro de sangre)?/giu;
    const percentagePattern = /(\d{1,3}(?:[.,]\d+)?)\s*%/giu;
    const millionPesosPattern = /(\d+(?:[.,]\d+)?)\s+millones?\s+de\s+pesos?/giu;
    const wordNumberPattern = /\b(un|uno|una|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\s+(aÃƒÂ±os|anos|aÃƒÂ±o|ano|meses|mes)\b/giu;
