function buildQueryPhraseMatches(candidate, queryTokens, normalizer) {
  ...
  const targetTokens = queryTokens.slice(0, 8);
  return {
    tokenMatches,
    phraseCoverage: targetTokens.length === 0 ? 1 : tokenMatches.filter((token) => targetTokens.includes(token)).length / targetTokens.length,
  };
}

export function computeSupportMetrics(candidate, { queryTokens, requiredFacts, normalizer, factNormalizer }) {
  const phraseSignals = buildQueryPhraseMatches(candidate, queryTokens, normalizer);
  ...
  const phraseCoverage = requiredFacts.length > 0 ? Math.max(0.4, phraseSignals.phraseCoverage) : phraseSignals.phraseCoverage;
  const structuredCoverage = requiredFacts.length > 0 ? (phraseCoverage + factCoverage) / 2 : phraseCoverage;
